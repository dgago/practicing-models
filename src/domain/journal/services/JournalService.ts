...
import { IDayStore } from "../db/IMonthStore";
...
export class JournalDomainService extends DomainService {
   ...
   * addNoteToDay
   */
  public async addNoteToDay(tenantId: string, date: Date, noteContents: string) {
    const id = DayEntity.getId(tenantId, date);
    const item = await this._dayStore.findOne(id);
   ...
   * addAppointmentToDay
   */
  public async addAppointmentToDay(tenantId: string, date: Date, appointmentDesc: string, appointmentTime: Date) {
    const id = DayEntity.getId(tenantId, date);
    const item = await this._dayStore.findOne(id);
   ...
   */
  public async removeAppointmentFromDay(tenantId: string, date: Date, appointment: Appointment) {
    const id = DayEntity.getId(tenantId, date);
    const item = await this._dayStore.findOne(id);
